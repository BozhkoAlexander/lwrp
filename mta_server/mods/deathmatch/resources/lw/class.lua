...
local function build(class, virtual_objs, is_virtual)
  ...
  -- Repository for storing virtual objects
  virtual_objs = virtual_objs or {}

  -- Virtual inheritance creates a single virtual child per base class
  if is_virtual and virtual_objs[class] then return virtual_objs[class] end
  ...
  -- If virtual, add it to the repository of virtual objects
...
function mt:__call(...)
  ...
  for i, base in ipairs(arg) do
    ...
    -- Get the base and whether it is inherited in virtual mode
    local basetype = typeof(base)
    local is_virtual = basetype == 'virtual_class'
    assert(basetype == 'class' or is_virtual, 
        'Base ' .. i .. ' is not a class or virtual class')
    if is_virtual then base = base.__class end
    ...
    for k, v in pairs(base) do
      ...
      if not reserved[k] and v ~= ambiguous and
        ...
        -- they originate in the same virtual class.
        elseif current_from.class ~= new_from.class or
            not current_from.virtual or not new_from.virtual then
